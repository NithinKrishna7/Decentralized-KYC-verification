...
contract KycBlockChain is KYC_Functions{
    ...
    function getAssociatedBank() public view returns (address, string memory) {
        require(isCus(),"You are not a customer");
        address bankAddress = allCustomers[msg.sender].bank_address;
        string memory bankName = allOrganisations[bankAddress].b_name;
        return (bankAddress, bankName);
    }
}